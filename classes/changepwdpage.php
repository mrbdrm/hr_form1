...
class ChangePasswordPage extends RunnerPage
{
	...
	protected function changePassword() {
		...
		$dbOldPass = "";
		if( !$row ) {
			$this->message = "كلمة السر غير صحيحة ";
			return false;
		}
	...
	protected function getPwdStrongFailedMessage()
	{
		...
		if($pwdLen)
		{
			$fmt = "يجب أن تكون كلمة المرورعلى الاقل ٪٪ طول أحرف.";
		...
		if($pwdUnique)
		{
			$fmt = "يجب أن تحتوي كلمة السر٪٪ شخصيات فريدة";
		...
		if($pwdDigits)
		{
			$fmt = "يجب أن تحتوي كلمة السر٪٪ أرقام أو رموز";
			$fmt = str_replace("%%", "".$pwdDigits, $fmt);
			$msg.= "<br>".$fmt;
		}
		if(GetGlobalData("pwdUpperLower", false))
		{
			$fmt = "يجب أن تحتوي كلمة المرور الأحرف في حالة العلوي والسفلي.";
			$msg.= "<br>".$fmt;
		}
